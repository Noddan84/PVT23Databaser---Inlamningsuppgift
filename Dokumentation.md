...
- URL: /api/load-test , /api/load-test2
- Metod: GET
- Beskrivning: Run-folder tester som hämtar alla böcker/författare
	där du kan välja antal iterationer, predefinierad gräns, antal användare, val av testtid,
	hur testerna körs.

### DATABAS UPPKOPPLINGSTEST
- URL: /api/books , i testkoden kan du ändra url utifrån eget tycke
- Metod: GET
- Beskrivning: Detta test syftar till att verifiera att anslutningen till vår databas är framgångsrik genom att skicka en förfrågan till vår databasserver och kontrollera responsen.
- Svarskoder:
- 200: Lyckad begäran.
...
- URL: /api/books
- Metod: GET
- Beskrivning: Skickar samtidigt flera förfrågningar för att hämta böcker/författare och testar responsen för att säkerställa att den är korrekt.
- Parametrar: numberOfRequests (antal samtidiga förfrågningar)
- Svarskoder:
- 200: Lyckad begäran.
